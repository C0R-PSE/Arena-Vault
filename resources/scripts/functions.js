...
export async function fetchPic(item) {
    var pathJson = await fetch('/resources/data/categoryPaths.json').then(resp => resp.json())
    var path = '/resources/images/items/' + pathJson[item.category] + '/' + item.shortName + '.webp'
    return(path)
}
...
export var itemsJson = await fetch('/resources/data/items.json').then(resp => resp.json())
export var tradesJson = await fetch('/resources/data/trades.json').then(resp => resp.json())
...
async function showTrade(inputName) {
    ...
    if (crafts != undefined) {
        ...
        for (var i in crafts) {
            ...
            traderPic.src = "/resources/images/contacts/" + trade.trader + ".png"
...
function showPreview(itemName) {
    ...
    valueElem.innerHTML = '<img class="koen" alt = "koen" src="/resources/styles/icons/koen.webp">' + item.value
...
export async function itemListClick(event, itemElem) {
...
export async function buildItem(item, settings) {
    ...
    itemPic.src = "/resources/images/items/Unknown.png"
    itemPic.setAttribute("onerror", "this.src='/resources/images/items/Unknown.png'")
    ...
    if (type == 'ingredient' || type == 'result') {
        ...
        itemValue.innerHTML = '<img class="koen" alt="koen" src="/resources/styles/icons/koen.webp">'
        if (item.value >= 10000) {
    ...
    //applying changes
    itemElem.addEventListener('click', (event) =>{itemListClick(event, itemElem)})
